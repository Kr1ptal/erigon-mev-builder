...
func applyTransaction(config *chain.Config, engine consensus.EngineReader, gp *GasPool, ibs *state.IntraBlockState,
	...
	evm *vm.EVM, cfg vm.Config, validationFunc func(*ExecutionResult) error) (*types.Receipt, []byte, error) {
	...
	// do some validation before finalization
	if validationFunc != nil {
		if err = validationFunc(result); err != nil {
			return nil, nil, err
		}
	}
...
func ApplyTransaction(config *chain.Config, blockHashFunc func(n uint64) libcommon.Hash, engine consensus.EngineReader,
	...
	return applyTransaction(config, engine, gp, ibs, stateWriter, header, tx, usedGas, usedBlobGas, vmenv, cfg, nil)
}

func ApplyTransactionWithValidation(config *chain.Config, blockHashFunc func(n uint64) libcommon.Hash, engine consensus.EngineReader, author *libcommon.Address, gp *GasPool, ibs *state.IntraBlockState, stateWriter state.StateWriter, header *types.Header, tx types.Transaction, usedGas, usedBlobGas *uint64, cfg vm.Config, validationFunc func(*ExecutionResult) error) (*types.Receipt, []byte, error) {
	// Create a new context to be used in the EVM environment
	...
	return applyTransaction(config, engine, gp, ibs, stateWriter, header, tx, usedGas, usedBlobGas, vmenv, cfg, validationFunc)
}
