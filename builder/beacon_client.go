...
func (b *testBeaconClient) getProposerForNextSlot(requestedSlot uint64) (PubkeyHex, error) {
	return PubkeyHex(hexutility.Encode(b.validator.Pk)), nil
}
...
func (m *MultiBeaconClient) SubscribeToPayloadAttributesEvents(payloadAttrC chan BuilderPayloadAttributes) {
	for _, c := range m.clients {
...
type PayloadAttributes struct {
	...
	SuggestedFeeRecipient libcommon.Address     `json:"suggested_fee_recipient"`
			...
			if err != nil {
				...
				for _, w := range payloadAttributesResp.Data.PayloadAttributes.Withdrawals {
					withdrawals = append(withdrawals, &types.Withdrawal{
						...
						Address:   libcommon.Address(w.Address),
						Amount:    uint64(w.Amount),
					})
				}

				data := BuilderPayloadAttributes{
